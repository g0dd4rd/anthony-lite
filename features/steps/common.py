...
def step_response_exact(context, text):
    assert context.result is not None, f"Expected response '{text}' but got None"
    assert context.result.strip() == text.strip(), f"Expected '{text}' but got: {context.result}"


@then("there is no match")
def step_no_match(context):
    assert context.result is None, f"Expected no match but got: {context.result}"


@then('MCP tool "{tool}" was called')
def step_mcp_called(context, tool):
    calls = context.mock_mcp.get_calls(tool)
    assert calls, f"MCP tool '{tool}' was not called. All calls: {context.mock_mcp.calls}"


@then('MCP tool "{tool}" was not called')
def step_mcp_not_called(context, tool):
    calls = context.mock_mcp.get_calls(tool)
    assert not calls, f"MCP tool '{tool}' was called but should not have been: {calls}"


@then('MCP tool "{tool}" was called with {key} "{value}"')
def step_mcp_called_with_str(context, tool, key, value):
    calls = context.mock_mcp.get_calls(tool)
    assert calls, f"MCP tool '{tool}' was not called"
    matched = any(c[1].get(key) == value for c in calls)
    assert matched, f"MCP tool '{tool}' was not called with {key}=\"{value}\". Calls: {calls}"
...
def step_mcp_called_with_generic(context, tool, key, value):
    ...
    assert matched, f"MCP tool '{tool}' was not called with {key}={expected}. Calls: {calls}"


@then('it spoke "{text}"')
def step_spoke(context, text):
    assert context.speaker.messages, f"Nothing was spoken. Expected to hear '{text}'"
...
def step_volume_muted(context):
    context.mock_mcp.responses["get_volume"] = lambda args: json.dumps({"volume": 0, "muted": True})


@given("the volume is at {level:d}")
def step_volume_at(context, level):
    context.mock_mcp.responses["get_volume"] = lambda args: json.dumps(
