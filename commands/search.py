...
from commands import step, _mcp_client, _smart_match_window, _get_friendly_app_name
...
@step('open {query}', 'launch {query}', 'start {query}',
      category='search', help_text='Open an application, file, or URL')
...
@step('go to {url}',
      category='search', help_text='Open a website in the browser')
def handle_go_to(context, url):
    return _open_url(url)


@step('find {query}', 'search for {query}',
      category='search', help_text='Search for files')
def handle_search_files(context, query):
    result = _mcp_client.call_tool("search_files", {
        "query": query, "file_type": "files", "limit": 5
    })
    try:
...
_FILE_EXTENSIONS = {
    'txt', 'pdf', 'doc', 'docx', 'odt', 'csv', 'py', 'js', 'ts', 'jsx',
    'png', 'jpg', 'jpeg', 'gif', 'svg', 'bmp', 'webp',
    'mp3', 'mp4', 'mkv', 'avi', 'wav', 'flac', 'ogg',
    'html', 'json', 'xml', 'md', 'sh', 'conf', 'log', 'yaml', 'yml',
    'zip', 'tar', 'gz', 'rpm', 'deb',
}
...
def _open_url(url):
    ...
    try:
        ...
        browser = _smart_match_window("firefox", win_list) or \
                  _smart_match_window("chromium", win_list) or \
                  _smart_match_window("chrome", win_list)
        if browser:
...
def _open_file(path):
    ...
    try:
        search_result = _mcp_client.call_tool("search_files", {
            "query": os.path.basename(path), "file_type": "files", "limit": 1
        })
