...
from commands import step, _mcp_client, _speak, _listen, _dialog_handler
...
@step('press {keys}', 'hit {keys}', 'push {keys}',
...
@step('click at {x:d} {y:d}',
      category='input', help_text='Click at screen coordinates')
def handle_click(context, x, y):
    _mcp_client.call_tool("mouse_click", {"x": x, "y": y, "button": 1})
    return f"Clicked at ({x}, {y})"


@step('double click at {x:d} {y:d}',
      category='input', help_text='Double-click at screen coordinates')
def handle_double_click(context, x, y):
    _mcp_client.call_tool("mouse_double_click", {"x": x, "y": y, "button": 1})
    return f"Double-clicked at ({x}, {y})"


@step('right click at {x:d} {y:d}',
      category='input', help_text='Right-click at screen coordinates')
...
@step('scroll down', 'scroll down {amount:d} times',
      category='input', help_text='Scroll down on the current window')
...
@step('scroll up', 'scroll up {amount:d} times',
      category='input', help_text='Scroll up on the current window')
...
@step('drag from {x1:d} {y1:d} to {x2:d} {y2:d}',
      category='input', help_text='Drag from one position to another')
def handle_drag(context, x1, y1, x2, y2):
    _mcp_client.call_tool("mouse_drag", {
        "x1": x1, "y1": y1, "x2": x2, "y2": y2, "button": 1
    })
