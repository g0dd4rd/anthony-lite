...
def handle_describe_screen(context):
    ...
    system_prompt = (
        "You are a screen reader for visually impaired users."
        " Describe what you see in plain text without any formatting."
...
def handle_describe_window(context):
    ...
    system_prompt = (
        "Describe what you see in this application window"
        " in plain text without any formatting."
        " Do not use markdown, asterisks, or special characters."
        " Answer directly."
    )
    description = _call_vision(
...
def handle_monitors(context):
    ...
    try:
        ...
        if len(monitors) == 0:
            ...
            scale = m.get("scale", 1)
            return f"1{primary_tag} monitor, resolution {m['width']}x{m['height']} at scale {scale}"
        else:
